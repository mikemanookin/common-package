function [frameTimes, frameRate] = getFrameTiming(frameMonitor, lcrFlag)
    ...
    if (lcrFlag == 0) % smooth out non-monotonicity (shows up on Rig G)
       frameMonitor = common.util.lowPassFilter(frameMonitor, 360, 1/1e4); 
    end
    ...
    ups = common.util.getThresCross(frameMonitor, 0.5, 1);
    downs = common.util.getThresCross(frameMonitor, 0.5, -1);
    ...
    if (lcrFlag == 1) % very fast, so frame times are clear directly from trace
        ...
        [~, flipsEven] = common.util.getPeaks(tempFM, 1);
        [~, flipsOdd] = common.util.getPeaks(tempFM, -1);
