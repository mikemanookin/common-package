classdef ChannelTest < symphonyui.core.Protocol
    ...
    methods
        ...
        function prepareEpoch(obj, epoch)
            ...
            % Opt in to ring buffer mode for this epoch's responses
            % devices = epoch.getResponseDeviceNames();
            % for i = 1:numel(devices)
            %     resp = epoch.getResponse(devices{i});
            %     resp.cobj.StreamingThreshold = System.TimeSpan.FromSeconds(10);
            %     resp.cobj.MaxDisplayChunks = 40;  % keep ~10 seconds for figures
            % end
            
            epoch.addStimulus(obj.rig.getDevice(obj.led), obj.createLedStimulus());
            epoch.addResponse(obj.rig.getDevice(obj.amp));
            resp = epoch.getResponse(obj.rig.getDevice(obj.amp));
            resp.cobj.StreamingThreshold = System.TimeSpan.FromSeconds(10);
            resp.cobj.MaxDisplayChunks = 40;  % keep ~10 seconds for figures
        end
